...
if __name__ == "__main__":
    ...
    training_data = []
    data = []
    with open(f"datasets/{args.dataset}.json",'r') as f:
        data = json.load(f)
    ...
    for sample in tqdm(data):
        if args.method == "unsure":
            ...
            if sample[1] in output: # Model is sure
                answer_text = sample[1]
            else: # Model is unsure
                answer_text = "I am unsure."
            
            training_data.append({"prompt": prompt_text, "answer": answer_text})
    ...
    output_file_path = f"./training_data/{args.result}_{args.method}.json"
    
    if args.method == "unsure":
        # For "unsure", training_data is a list of {"prompt": ..., "answer": ...}
        # Dump this list directly to the JSON file.
        with open(output_file_path, 'w') as f:
            json.dump(training_data, f) 
    else:
        # For other methods, training_data contains {"text": ...} items.
        # Use the original LMFlow_data structure.
